...
function mapTickerToPair(ticker) {
    const map = {
...
function mapUiWindowToStat(ui, sinceMinutes) {
    if (ui === '24h')
        return 'last24hours';
    if (ui === '7d')
        return 'last7days';
    ...
    if (sinceMinutes <= 1440)
        return 'last24hours';
    if (sinceMinutes <= 10080)
        return 'last7days';
    return 'last30days';
}
async function tryFetchStat(ticker, dateParam) {
    if (!CRYPTONEWS_API_KEY) {
        return { ok: false, score: 0, count: 0, bullish: 0, bearish: 0, drivers: [] };
    }
    const url = `https://cryptonews-api.com/api/v1/stat?tickers=${encodeURIComponent(ticker)}&date=${dateParam}&page=1&token=${CRYPTONEWS_API_KEY}`;
    try {
...
async function fetchHeadlinesForTicker(ticker, sinceMinutes) {
    if (!CRYPTONEWS_API_KEY)
        return [];
...
function aggregateHeadlineScores(items) {
    const now = Date.now();
...
async function fetchCryptoNews(tickers, sinceMinutes = 1440, opts) {
    ...
    if (!res.ok)
        throw new Error(`CryptoNews API error ${res.status}`);
    const data = await res.json();
    const articles = data?.data || data?.news || [];
    const mapped = articles.map((a) => {
        const id = String(a.news_url || a.id || a.url || globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`);
        const title = a.title;
        ...
        else if (prov === 'negative') {
            sentiment = 'bearish';
            score = -0.3;
        }
        else if (prov === 'neutral') {
            sentiment = undefined;
            score = 0;
        }
...
async function classifySentimentOpenAI(texts) {
    ...
    try {
        ...
        return texts.map((_, i) => {
            ...
            const sentiment = sRaw === 'bearish' ? 'bearish' : 'bullish';
            let score = Number(it?.score);
            if (!Number.isFinite(score))
                score = sentiment === 'bullish' ? 0.1 : -0.1;
...
function computeFeaturesFromCloses(closes) {
...
async function fetchPriceFeaturesCoingecko(ticker, tf) {
    const idMap = {
    ...
    const daysMap = { '15m': 1, '1h': 1, '4h': 2, '1d': 7 };
...
function aggregateSentiment(items) {
    ...
    if (!n)
        return { avg: 0, bullish: 0, bearish: 0 };
...
app.get('/api/news', async (req, res) => {
    try {
        const tickerParam = req.query.tickers || req.query.ticker || 'BTC';
        ...
        const sentimentRaw = req.query.sentiment?.toLowerCase();
        const sentiment = (sentimentRaw === 'positive' || sentimentRaw === 'negative' || sentimentRaw === 'neutral') ? sentimentRaw : undefined;
        const raw = await fetchCryptoNews(tickers, since, { sentiment: sentiment, items, page });
        const toClassifyIdx = [];
        const texts = [];
        raw.forEach((r, i) => { if (!r.sentiment) {
            toClassifyIdx.push(i);
            texts.push(r.title);
        } });
        let labels = [];
        if (texts.length)
            labels = await classifySentimentOpenAI(texts);
...
app.get('/api/signal', async (req, res) => {
    try {
        ...
        try {
            ...
            price = { source: 'unavailable', last: 0, momentum: 0, changePct: 0, vol: 0 };
        }
...
app.post('/api/analyze', async (req, res) => {
    try {
        ...
        const news = Array.isArray(body.news) && body.news.length
            ? body.news.map((n, idx) => ({
                id: String(idx),
        ...
        const toClassifyIdx = [];
        const texts = [];
        news.forEach((r, i) => { if (!r.sentiment) {
            toClassifyIdx.push(i);
            texts.push(r.title);
        } });
        let labels = [];
        ...
        let chartReasons = [], newsReasons = [];
        const priceAvailable = price.source !== 'unavailable';
        if (priceAvailable) {
            if (totalScore > 0.1 && price.momentum > 0) {
                status = 'BUY';
                confidence = Math.min(88, 55 + Math.round((Math.abs(totalScore) * 30 + price.momentum) / 2));
            }
            else if (totalScore < -0.1 && price.momentum < 0) {
                status = 'SELL';
                confidence = Math.min(88, 55 + Math.round((Math.abs(totalScore) * 30 + Math.abs(price.momentum)) / 2));
            }
            else {
                status = 'NEUTRAL';
                confidence = 45;
            ...
            if (totalScore > 0.15) {
                status = 'BUY';
                confidence = Math.min(80, 50 + Math.round(Math.abs(totalScore) * 35));
            }
...
app.get('/api/stat/general', async (req, res) => {
    try {
        ...
        if (!r.ok)
            return res.json({ score: 0, sentiment: 'neutral', items: 0 });
...
app.get('/api/news/general', async (req, res) => {
    try {
        ...
        if (!resPositive.ok)
            throw new Error(`CryptoNews positive error ${resPositive.status}`);
        if (!resNegative.ok)
            throw new Error(`CryptoNews negative error ${resNegative.status}`);
        const dataPositive = await resPositive.json();
        const dataNegative = await resNegative.json();
        const articlesPositive = dataPositive?.data || dataPositive?.news || [];
        const articlesNegative = dataNegative?.data || dataNegative?.news || [];
        // Map both sets of articles
        const mapArticle = (a) => ({
            id: String(a.news_url || a.id || a.url || globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`),
            title: a.title,
        ...
        const mappedPositive = articlesPositive.map(mapArticle);
        const mappedNegative = articlesNegative.map(mapArticle);
        // Filter valid articles
        const filterValid = (m) => m.url && m.url.startsWith('http') && m.source && m.source !== 'Unknown';
        ...
        const labeledPositive = filteredPositive.map((r, i) => ({
            ...r,
            sentiment: 'bullish', // Force bullish for positive news
            score: labelsPositive[i]?.score ?? 0.3
        }));
        const labeledNegative = filteredNegative.map((r, i) => ({
            ...r,
            sentiment: 'bearish', // Force bearish for negative news
